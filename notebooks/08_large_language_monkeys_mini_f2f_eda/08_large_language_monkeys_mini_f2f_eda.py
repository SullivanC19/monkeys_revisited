...
large_language_monkeys_code_contests_pass_at_k_df = src.analyze.create_or_load_large_language_monkeys_mini_f2f_pass_at_k_df(
    refresh=False,
    # refresh=True,
)
...
g = sns.lineplot(
    ...
    y="Score",
    hue="Model",
    hue_order=src.globals.LARGE_LANGUAGE_MONKEYS_MINI_F2F_MODELS_ORDER,
    style="Benchmark",
)
g.set(
...
g = sns.relplot(
    ...
    style="Benchmark",
    col="Model",
    col_order=src.globals.LARGE_LANGUAGE_MONKEYS_MINI_F2F_MODELS_ORDER,
    col_wrap=3,
    estimator=None,
)
g.set(
...
for ax, model in zip(
    g.axes.flat, src.globals.LARGE_LANGUAGE_MONKEYS_MINI_F2F_MODELS_ORDER
):
...
g.fig.suptitle("Large Language Monkeys (Benchmark = MiniF2F-MATH)")
g.fig.subplots_adjust(top=0.9)
src.plot.save_plot_with_multiple_extensions(
    plot_dir=results_dir,
...
print("Finished notebooks/08_large_language_monkeys_mini_f2f_eda!")
