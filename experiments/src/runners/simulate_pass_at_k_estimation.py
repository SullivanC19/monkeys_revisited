...
def compute_true_pass_at_k(samples: list[list[bool]], k_values: list[int]) -> list[float]:
    """
    ...
    """
    hardness = np.array([sum(st_samples) / len(st_samples) for st_samples in samples])
    return [(1 - np.mean((1 - hardness) ** k)).item() for k in k_values]
...
def run_single(
    ...
    """
    pass_at_k = compute_true_pass_at_k(samples, K_VALUES)
    ...
    return [{
        ...
        "Regression Num. Attempts": regression_n_attempts,
        "Regression Num. Successes": regression_n_successes,
        "Discretization Estimate": discretization_estimates[i],
        "Discretization Num. Attempts": discretization_n_attempts,
        "Discretization Num. Successes": discretization_n_successes,
        "Dynamic Estimate": dynamic_estimates[i],
        "Dynamic Num. Attempts": dynamic_n_attempts,
        "Dynamic Num. Successes": dynamic_n_successes,
