...
from .simulate_pass_at_k_estimation import (
    simulate_dynamic_estimate_of_pass_at_k,
    simulate_uniform_estimate_of_pass_at_k,
    compute_hardness,
...
def run():
    ...
    for budget in tqdm(BUDGET_VALUES):
        for seed in SEEDS[:20]:
            ...
            dynamic_estimates, _, _ = simulate_dynamic_estimate_of_pass_at_k(
                samples=samples,
                budget=budget,
                k_values=[k],
                seed=seed,
            )
            uniform_estimates, _, _ = simulate_uniform_estimate_of_pass_at_k(
                samples=samples,
                budget=budget,
                k_values=[k],
                seed=seed,
